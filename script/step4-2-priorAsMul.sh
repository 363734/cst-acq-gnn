...
outputdir="${workingdir}/logs/resultsCA/${caalgoname}/pam"
mkdir -p ${outputdir}
...
for idx in "${!is[@]}"; do
  ...
  for arch in "nn" "rf"; do
    echo " - with arch ${arch}"
    for lam in "0.25" "0.5" "0.75" "1"; do
      echo " - with lambda ${lam}"

      priorname="${arch}_allbut_${modnameprior}"
      logfile="${outputdir}/log_withprior_pam_${lam}_p[${priorname}]_b[${modnameprior}].txt"
      priorfile="${workingdir}/models/${arch}/${priorname}.pickle"

      echo "   - with prior ${priorname}"
      python ${scriptdir}/cstAcqAlgos/main.py -a ${caalgo} -b ${modnameca} -pam -lam ${lam} -pf ${priorfile} > ${logfile}
