...
for idx in "${!is[@]}"; do
  ...
  do
    ...
    do
      instancenameCA=${js[$idx]}
      printf "${jobid2}\t${caalgoname}\t${caalgo}\t${modelnames[$i]}\t${instancenameCA}\t${runid}\t\n" >> "${outputdir}/${outputfile2}"
      jobid2=$((jobid2 + 1))
    done
  done
done
